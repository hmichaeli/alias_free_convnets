...
STEPS = {
    'translation': attack_steps.Translation,
...
class Attacker(ch.nn.Module):
    ...
    """
    def __init__(self, model, dataset, attack_space):
        """
    ...
    def forward(self, x, target, *_, tries, spatial_constraint, attack_type,
                ...
                orig_input=None, use_best=True, translation_grid_size=64):
        """
        ...
        if self.attack_space == 'translation':
            step = STEPS[self.attack_space](attack_type=attack_type,
                                            spatial_constraint=spatial_constraint,
                                            grid_size=translation_grid_size)
        else:
            step = STEPS[self.attack_space](attack_type=attack_type,
                                            spatial_constraint=spatial_constraint)
...
class AttackerModel(ch.nn.Module):
    ...
    """
    def __init__(self, model, dataset, attack_space='spatial'):
        super(AttackerModel, self).__init__()
        self.normalizer = helpers.InputNormalize(dataset.mean, dataset.std)
        self.model = model
        self.attacker = Attacker(model, dataset, attack_space)
