...
class ImageNetZeroPadding(ImageNet):
    def __init__(self, data_path, zp, **kwargs):
        ds_kwargs = {
            'num_classes': 1000,
            'mean': ch.tensor([0.485, 0.456, 0.406]),
            'std': ch.tensor([0.229, 0.224, 0.225]),
            ...
            'label_mapping': None,
            'transform_train': da.TRAIN_TRANSFORMS_IMAGENET,
            'transform_test': da.TEST_TRANSFORMS_IMAGENET_ZERO_PADDING(zp)
        }
...
DATASETS = {
    ...
    'imagenet_zero_padding': ImageNetZeroPadding,
