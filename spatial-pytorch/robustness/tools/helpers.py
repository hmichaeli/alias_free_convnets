...
def accuracy(output, target, topk=(1,), exact=False):
    ...
    with torch.no_grad():
        ...
        res_exact = []
        for k in topk:
            # [bugfix] RuntimeError: view size is not compatible with input tensor's size and stride (at least one dimension spans across two contiguous subspaces). Use .reshape(...) instead.
