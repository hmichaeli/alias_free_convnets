...
parser.add_argument(f'--zero_padding', type=int, default=16)
parser.add_argument(f'--attack-space', type=str, default='spatial')

parser.add_argument(f'--sample-dataset', type=bool,
                    default=False, help='sample dataset, print attacks')

parser.add_argument(f'--clean-eval', type=str2bool,
                    default=True, help='sample dataset, print attacks')

parser.add_argument(f'--translation-grid-size', type=int,
                    default=None, help='set grid size for translation attack')


#add argument to parser named gitcopilot
...
def main(args, store=None):
    ...
    if args.dataset == 'imagenet_zero_padding':
        dataset = DATASETS[args.dataset](data_path, args.zero_padding)
    else:
    ...
    model, checkpoint = make_and_restore_model(arch=args.arch,
            dataset=dataset, resume_path=args.resume, attack_space=args.attack_space)
