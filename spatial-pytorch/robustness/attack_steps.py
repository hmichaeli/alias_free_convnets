...
class Spatial:
    ...
    def step(self, x, g, correcter=None):
        ...
        while i < ntfm:
            ...
            # RuntimeError: "argmin_cuda" not implemented for 'Bool'
            # [bugfix] cast is_coorect from bool to float
            argmin = (is_correct * 1.0).argmin()
...
class Translation:
    def __init__(self, attack_type, spatial_constraint, grid_size=64):
        self.use_grad = False
        self.rot_constraint = 0.0
        self.trans_constraint = (float(spatial_constraint)/10.)/28.
        self.attack_type = attack_type
    ...
    def step(self, x, g, correcter=None):
        ...
        elif self.attack_type == 'random_int':
            rots = spatial.unif((bs,), -max_rot, max_rot) # zeros

            txs = spatial.unif_int((bs, 2), -max_trans * 280, max_trans * 280)
            # print('txs', txs)
            # print('rots', rots)
            txs = txs / 280.0
        ...
        assert self.attack_type == 'grid'
        assert x.shape[0] == 1 , '[hm] batch size should be 1 on each device (?)'

        # rots = ch.linspace(-max_rot, max_rot, steps=NUM_ROT)
        rots = ch.zeros((bs,))
        trans = ch.linspace(-max_trans, max_trans, steps=self.grid_size)
        tfms = ch.tensor(list(itertools.product(rots, trans, trans))).cuda(device=device)
        ...
        while i < ntfm:
            ...
            # print(is_correct)
            # RuntimeError: "argmin_cuda" not implemented for 'Bool'
            # [hm] cast is_coorect from bool to float
            argmin = (is_correct * 1.0).argmin()
            if is_correct[argmin] == 0:
                return transformed[i+argmin:i+argmin+1]
