...
class ConvNeXt(nn.Module):
    ...
    def __init__(self, in_chans=3, num_classes=1000, 
                ...
                activation='gelu', activation_kwargs={}
                 ):
        super().__init__()

        self.downsample_layers = nn.ModuleList() # stem and 3 intermediate downsampling conv layers
        stem = nn.Sequential(
            conv4x4(in_chans, dims[0], stride=4, conv_pad_type=conv_pad_type),
        ...
        for i in range(3):
            downsample_layer = nn.Sequential(
                    ...
                    conv2x2(dims[i], dims[i + 1], stride=2, conv_pad_type=conv_pad_type),
            )
